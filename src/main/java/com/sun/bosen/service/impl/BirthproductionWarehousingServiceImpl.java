...
public class BirthproductionWarehousingServiceImpl implements BirthproductionWarehousingService{
	...
	public String add(PP_Pomain[] data) {
		...
		for (int i = 0; i < data.length; i++) {
			...
			//更新currentStock
			currentStockService.updateCurrentStock(
					data[i].getcWhCode(), data[i].getInventory().getcInvCode(),data[i].getNowiReceivedQTY()
			);
			myCurrentStockService.updateCurrentStock(
					data[i].getcWhCode(), data[i].getInventory().getcInvCode(),data[i].getNowiReceivedQTY()
			);
